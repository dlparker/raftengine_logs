...
async def test_combi_specific():
    ...
    # get pruned from lmdb. The combi log keeps track
    ...
    try:
        ...
        for loopc in range(10):
            ...
            threashold_index = start_index + log.high_limit
            ...
            while next_index < threashold_index:
                ...
                rec = await log.append(new_rec)
                next_index += 1
            # make sure we are at the threashol, not passed it due to fenchpost
            assert start_index == await log.get_first_index()
            assert next_index - start_index == log.high_limit
