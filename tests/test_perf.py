...
async def test_lmdb_perf():
    ...
    lmdb_times = await inner_log_perf_run(lmdb_log_create)
    print('\nHYBRID')
    hybrid_times = await inner_log_perf_run(hybrid_log_create)
    ...
    print(f"\n{'Type':12s} ")
    print(f"{'Memory':12s} = {m_p_loop:.6f} Memory x {1.0:.6f}")
    print(f"{'LMDB':12s} = {l_p_loop:.6f} Memory x {lcost:.6f}")
    print(f"{'HYBRID':12s} = {h_p_loop:.6f} Memory x {hcost:.6f}")
    print(f"{'Sqlite':12s} = {s_p_loop:.6f} Memory x {scost:.6f}")
